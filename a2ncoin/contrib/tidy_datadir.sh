...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete A2ncoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no A2ncoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old A2ncoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected A2ncoin 0.7 datadir."
    ;;
  3)
    echo "Detected A2ncoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected A2ncoin 0.8 datadir."
    ;;
esac
