...
###   This script attempts to download the signature file SHA256SUMS.asc from a2ncoin.org
...
RCSUBDIR="test"
BASEDIR="https://a2ncoin.org/bin/"
VERSIONPREFIX="a2ncoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'a2ncoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the A2ncoin Core binary release signing key installed?"
   fi
