...
The RPM spec file provided here is for A2ncoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://a2ncoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[A2ncoin git repository](https://github.com/a2ncoin/a2ncoin) but are not part of
...
Currently only `Source10` is used, for the example `a2ncoin.conf` file.
...
in the a2ncoin git repository. At present only an SVG version of the A2ncoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the a2ncoin
...
needed when 0.12.1 is released, a proper fix is already in the A2ncoin git
master branch.
...
idea to build A2ncoin Core against a static version of that library compiled
...
The `a2ncoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `a2ncoin-server` package. At this point, most RPM based
...
The `a2ncoin-server` package should have SELinux support. How to properly do
...
A2ncoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/a2ncoin/). He has been
packaging A2ncoin for Fedora at least since 2012.
...
4. I renamed the `a2ncoin` package that contains the Qt GUI to `a2ncoin-core` as
that appears to be how the general population refers to it, in contrast to
`a2ncoin-xt` or `a2ncoin-classic`. I wanted to make sure the general population
