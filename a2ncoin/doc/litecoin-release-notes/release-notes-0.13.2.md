A2ncoin Core version 0.13.2 is now available from:

  <https://download.a2ncoin.org/a2ncoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/a2ncoin-project/a2ncoin/issues>
...
updates will be released anymore. Without security updates, using a a2ncoin
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of A2ncoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/a2ncoin/a2ncoin/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside A2ncoin transactions now use validation using
[libsecp256k1](https://github.com/a2ncoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/a2ncoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of A2ncoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of A2ncoin Core was to raise the default minimum
relay fee.

A2ncoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
A2ncoin Core 0.13.2 also introduces new default policy limits on the length and
...
A2ncoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. A2ncoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In A2ncoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in A2ncoin Core 0.13, and it is at this time undecided whether the
...
A2ncoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
A2ncoin Core automatically creates a hidden service to listen on, without
manual configuration. A2ncoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if A2ncoin Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals A2ncoin
Core to use floating fees. By default, A2ncoin Core will use floating
fees.
...
At all times, A2ncoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, A2ncoin Core will never create transactions paying less than
...
`a2ncoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of A2ncoin Core v0.11.0](https://github.com/a2ncoin/a2ncoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/a2ncoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/a2ncoin/bips/blob/master/bip-0065.mediawiki)
...
- Adding `dbcache=100` in a2ncoin.conf
...
For example:
    ...
    $ src/a2ncoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The A2ncoin Core code base has
...
- `a2ncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `a2ncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/a2ncoin/a2ncoin/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the A2ncoin scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/a2ncoin/a2ncoin/pull/7524>
...
A2ncoin transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/a2ncoin/a2ncoin/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/a2ncoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of A2ncoin Core.

[Pull request](https://github.com/a2ncoin/a2ncoin/pull/8035/files), [BIP 32](https://github.com/a2ncoin/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/a2ncoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/a2ncoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the A2ncoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/a2ncoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/a2ncoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/a2ncoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/a2ncoin/a2ncoin/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming A2ncoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for A2ncoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each A2ncoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** A2ncoin addresses (both P2PKH addresses
  ...
  believed to be provided by A2ncoin's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  A2ncoin paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of A2ncoin's current ledger (called the UTXO set),
  ...
  note that A2ncoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  A2ncoin users to individually opt-in to almost any change in the A2ncoin
  Script language when those users receive new transactions.  Features
  currently being researched by A2ncoin and A2ncoin Core contributors that may
  ...
  Other A2ncoin community members are studying several other improvements
...
segwit's start date of 1 January 2017 miners can update the A2ncoin
client to A2ncoin Core 0.13.2 to signal for segwit support. When a
...
[BIP141]: https://github.com/a2ncoin/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/a2ncoin/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/a2ncoin/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/a2ncoin/bips/blob/master/bip-0145.mediawiki
...
Since A2ncoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/a2ncoin/bips/blob/master/bip-0147.mediawiki
...
- `a2ncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `a2ncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- [The A2ncoin Core Developers](/doc/release-notes)
- Charles Lee
- Adrian Gallagher
- shaolinfry
- Xinxi Wang
