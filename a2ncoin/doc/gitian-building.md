...
*Setup instructions for a Gitian build of A2ncoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the A2ncoin
...
- [Building A2ncoin Core](#building-a2ncoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-a2ncoin-host/README.md).
...
Clone the git repositories for a2ncoin and Gitian.
...
in the a2ncoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building A2ncoin Core
----------------

To build A2ncoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the a2ncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/a2ncoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/a2ncoin-project/a2ncoin
...
For example:
```bash
URL=https://github.com/thrasher-/a2ncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit a2ncoin=${COMMIT} --url a2ncoin=${URL} ../a2ncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit a2ncoin=${COMMIT} --url a2ncoin=${URL} ../a2ncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit a2ncoin=${COMMIT} --url a2ncoin=${URL} ../a2ncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the a2ncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../a2ncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/a2ncoin
SIGPATH=/some/root/path/a2ncoin-detached-sigs

./bin/gbuild --url a2ncoin=${BTCPATH},signature=${SIGPATH} ../a2ncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/a2ncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/a2ncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/a2ncoin-osx-build.assert
```
...
[a2ncoin-project/gitian.sigs.ltc](https://github.com/a2ncoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
