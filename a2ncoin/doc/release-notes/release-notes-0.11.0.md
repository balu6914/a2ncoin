A2ncoin Core version 0.11.0 is now available from:

  <https://a2ncoin.org/bin/a2ncoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer A2N/kB fee to be
...
For example, add the following to `a2ncoin.conf`:
...
related to the blockchain in the a2ncoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows A2ncoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that A2ncoin Core will 
...
For an example script see [a2ncoin-submittx](https://github.com/laanwj/a2ncoin-submittx).
...
- #5505 `a99ef7d` Build winshutdownmonitora2ncoin.cpp on Windows only
...
- #6269 `95aca44` gitian: Use the new a2ncoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add a2ncoin logo to about screen
...
- #5809 `46bfbe7` Add a2ncoin-cli man page
...
- #6116 `30dc3c1` [OSX] rename A2ncoin-Qt.app to A2ncoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for a2ncoin-tx)
